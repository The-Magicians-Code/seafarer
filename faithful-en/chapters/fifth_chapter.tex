...
The main work happens in the inference script (see the simplified Figure~\ref{fig:image16} and the technical Figure~\ref{fig:image18}), to which the user provides as input the neural-network model and its required parameters, the input stream (a video file or a camera), and, if needed, the desired mode (video recording, or a mode without video output intended for debugging and speed checks) (see Figure~\ref{fig:image17}).

\begin{figure}[H]
\centering
\includegraphics[width=6.00897in,height=4.07576in]{media/image16.png}
\caption{Simplified block diagram of the main program}\label{fig:image16}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=6.10174in,height=0.33333in]{media/image17.png}
\caption{Example of running the main program from the command line}\label{fig:image17}
\end{figure}
...
\caption{Simplified block diagram of the computer-vision program's operation}\label{fig:image18}
\end{figure}

To detect the machine architecture, a Python module is used; based on its output, the required settings are applied for the environment that ran the script. The development environment is a desktop computer with an x86\_64 architecture; during setup the corresponding video decoder and encoder are initialised, the special-use port is set, and a scaling parameter for the information coming from the cameras is configured (see Figure~\ref{fig:image19}).

\begin{figure}[H]
\centering
\includegraphics[width=4.21875in,height=1.96875in]{media/image19.png}
\caption{Initialisation of architecture-specific parameters}\label{fig:image19}
\end{figure}
...
Frames received from the cameras are scaled to an input format suitable for the neural network, after which the model extracts information from the frames and formulates statistical outputs. The output information is then sent to post-processing, where the results are formatted and the locations of detected objects on the frame are displayed along with their confidence probabilities (see Figure~\ref{fig:image20}). A new frame is then read in, and the program's work repeats until no more frames come in -- i.e.\ either the camera stops working or the user terminates the program.

\begin{figure}[H]
\centering
\includegraphics[width=5.98021in,height=3.72487in]{media/image20.png}
\caption{Detections generated and processed by the neural network, displayed on a frame (bounding boxes and the detected classes inside them together with the probabilities)}\label{fig:image20}
\end{figure}
...
A second problem was that larger computer-vision models could not be run successfully, because the models were loaded into CPU memory and a marginal portion of the computational work, alongside frame processing in the neural network, fell on the GPU (see Figure~\ref{fig:image21}).

\begin{figure}[H]
\centering
\includegraphics[width=5.94792in,height=4.05208in]{media/image21.png}
\caption{Running YOLOv5l, with a single input at 640$\times$640-pixel resolution, in unoptimised form does not utilise the whole GPU, as seen in the monitoring tool jtop \cite{ref43}}\label{fig:image21}
\end{figure}
...
As a result of the optimisations (see Figure~\ref{fig:image22}):
...
\caption{Running the YOLOv5l model, with a single input at 640$\times$640-pixel resolution, in optimised form utilises the whole GPU}\label{fig:image22}
\end{figure}
